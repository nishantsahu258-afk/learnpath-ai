  ...
  return (
    <div className="max-w-4xl mx-auto space-y-8 pb-12">
      ...
      <div className="relative mt-6 sm:mt-8">
        {/* Responsive Vertical Line (Centered to 40px node on mobile, 56px on desktop) */}
        <div className="absolute left-[19px] sm:left-[27px] top-4 bottom-4 w-0.5 bg-slate-800" />
        ...
        <div className="space-y-6 sm:space-y-8">
          {learningPath.modules.map((module, index) => {
            ...
            if (isComplete) {
              statusIcon = <CheckCircle size={20} weight="duotone" className="text-cyan-400 sm:w-6 sm:h-6" />;
              statusBadge = <span className="text-[11px] sm:text-xs font-bold text-cyan-400 bg-cyan-400/10 px-2 py-0.5 sm:py-1 rounded-md uppercase tracking-wide">Completed</span>;
            } else if (isCurrentModule) {
              statusIcon = <CircleNotch size={20} weight="bold" className="text-fuchsia-400 animate-spin sm:w-6 sm:h-6" />;
              statusBadge = <span className="text-[11px] sm:text-xs font-bold text-fuchsia-400 bg-fuchsia-400/10 px-2 py-0.5 sm:py-1 rounded-md uppercase tracking-wide">In Progress</span>;
              borderClass = 'border-fuchsia-500/30 ring-1 ring-fuchsia-500/10 shadow-[0_0_15px_rgba(217,70,239,0.05)]';
            } else {
              statusIcon = <div className="w-3 h-3 sm:w-4 sm:h-4 rounded-full border-2 border-slate-600 m-1" />;
              statusBadge = <span className="text-[11px] sm:text-xs font-bold text-slate-500 bg-slate-800 px-2 py-0.5 sm:py-1 rounded-md uppercase tracking-wide">Upcoming</span>;
            }

            const firstUncompletedLesson = module.lessons.find(l => !learningProgress.completedLessonIds.includes(l.id)) || module.lessons[0];

            return (
              <div key={module.id} className="relative flex items-start gap-3.5 sm:gap-6">
                {/* Timeline Node (40px on mobile, 56px on desktop) */}
                <div className="relative z-10 shrink-0 w-10 h-10 sm:w-14 sm:h-14 bg-slate-950 rounded-full border-2 sm:border-4 border-slate-950 flex items-center justify-center">
                ...
                <div className="flex-1 min-w-0 pt-0.5 sm:pt-1">
                  <Card className={`p-4 sm:p-6 transition-all ${borderClass} overflow-hidden relative group`}>
                    <div className="flex flex-col md:flex-row md:items-start justify-between gap-4">
                      
                      <div className="flex-1">
                        <div className="flex items-center gap-2.5 sm:gap-3 mb-1.5 sm:mb-2">
                          <span className="text-xs sm:text-sm font-bold text-slate-500">MODULE {index + 1}</span>
                          {statusBadge}
                        </div>
                        <h3 className="text-base sm:text-xl font-bold text-white mb-2 leading-snug break-words">{module.title}</h3>
                      ...
                      {/* Action Button (Full-width on mobile, comfortable 44px touch height) */}
                      <div className="shrink-0 w-full md:w-auto mt-2 md:mt-0">
                        {isCurrentModule ? (
                          <Link to={`/lesson/${module.id}/${firstUncompletedLesson.id}`} state={{ from: '/learning-path' }} className="block w-full">
                            <Button variant="gradient" className="w-full min-h-[44px] shadow-lg shadow-fuchsia-500/20">
                              <Play size={16} weight="fill" className="mr-2" /> 
                              {completedLessonsInModule > 0 ? 'Continue' : 'Start'}
                            </Button>
                          </Link>
                        ) : isComplete ? (
                          <Link to={`/lesson/${module.id}/${module.lessons[0].id}`} state={{ from: '/learning-path' }} className="block w-full">
                            <Button variant="outline" className="w-full min-h-[44px] border-cyan-500/30 text-cyan-400 hover:bg-cyan-500/10">
                              Review
                            </Button>
                          </Link>
                        ) : (
                          <Link to={`/lesson/${module.id}/${module.lessons[0].id}`} state={{ from: '/learning-path' }} className="block w-full">
                            <Button variant="ghost" className="w-full min-h-[44px] text-slate-400 hover:text-white flex items-center justify-center gap-1">
